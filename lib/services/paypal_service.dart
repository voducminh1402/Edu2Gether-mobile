...
class PaypalService {
  ...
  Map<String, String> headers = {
    ...
    "Authorization":
        'Basic QVpyWmw1UEJJc1R3Qk56ZFdSaTM0UkpGYVFFWElsZmphN2p1ZzF1M3dmZUt1WDVJR2dZeDVwc3VNaUU5eXdCWnNMai0wTWt4T2dLeU5WYjk6RU1WTENfZ2NuQlgza0QwcFFiZkFQYV9MbGFqLW9uc0RwVU5rZk9wVDUtUHE4bVdoeC1RM1BadTZ0M2l1WVRpSGktRndmNlVWN0FCcVgtQXo=',
  };
  ...
  Future<Map<String, String>> createCheckOutOrder(transaction) async {
    try {
      var response = await http.post(Uri.parse('$domain/v2/checkout/orders'),
          headers: headers, body: jsonEncode(transaction));
  ...
  Future<String> authorizePaymentOrder(
      orderId, mentorId, menteeId, courseId, price) async {
    try {
      ...
      final body = convert.jsonDecode(response.body);
      if (response.statusCode == 201) {
        if (body["purchase_units"] != null &&
            body["purchase_units"].length > 0) {
          List links = body["purchase_units"][0]['payments']['authorizations']
              [0]['links'];
          String capture = "";
  ...
  Future<String> capturePayment(
      url, mentorId, menteeId, courseId, price) async {
    try {
      var response = await http.post(
        Uri.parse(url),
        headers: headers,
